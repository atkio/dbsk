...
void CALLBACK_SetupExtra(Bitu callback, Bitu type, PhysPt physAddress, bool use_cb=true) {
	if (callback>=CB_MAX)
		return;
	switch (type) {
	case CB_RETN:
...
__pure static Bitu CALLBACK_SetupExtra_RTN(Bitu type,  bool use_cb=true) {
	switch (type) {
	case CB_RETN:
		return (use_cb?5:1);
	case CB_RETF:
...
#if HAVE_NEON
	CALLBACK_SetupExtra(callback,type,addr,(handler!=NULL));
	Bitu csize=CALLBACK_SetupExtra_RTN(type,(handler!=NULL));
#else
