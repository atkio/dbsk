...
static  INLINE void FPU_FCLEX(void){
	fpu.sw &= 0x7f00;			//should clear exceptions
}

static INLINE void FPU_FNOP(void){
	return;
}

static INLINE void FPU_PUSH(double in){
	TOP = (TOP - 1) &7;
...
static INLINE void FPU_PREP_PUSH(void){
	TOP = (TOP - 1) &7;
	fpu.tags[TOP] = TAG_Valid;
}

static INLINE void FPU_FPOP(void){
...
static INLINE Real64 FPU_FLD80(PhysPt addr) {
//TODO
		struct {
			Bit16s begin;
			FPU_Reg eind;
...
static INLINE void FPU_FLD_F32(PhysPt addr,Bitu store_to) {
	union {
		float f;
		Bit32u l;
	}	blah;
...
static INLINE void FPU_FLD_F64(PhysPt addr,Bitu store_to) {
	fpu.regs[store_to].l.lower = mem_readd(addr);
	fpu.regs[store_to].l.upper = mem_readd(addr+4);
}

static INLINE void FPU_FLD_F80(PhysPt addr) {
	fpu.regs[TOP].d = FPU_FLD80(addr);
}

static INLINE void FPU_FLD_I16(PhysPt addr,Bitu store_to) {
	Bit16s blah = mem_readw(addr);
	fpu.regs[store_to].d = static_cast<Real64>(blah);
}

static INLINE void FPU_FLD_I32(PhysPt addr,Bitu store_to) {
	Bit32s blah = mem_readd(addr);
	fpu.regs[store_to].d = static_cast<Real64>(blah);
}

static INLINE void FPU_FLD_I64(PhysPt addr,Bitu store_to) {
	FPU_Reg blah;
...
static INLINE void FPU_FBLD(PhysPt addr,Bitu store_to) {
	Bit64u val = 0;
	Bitu in = 0;
	Bit64u base = 1;
	for(Bitu i = 0;i < 9;i++){
...
static INLINE void FPU_FST_F32(PhysPt addr) {
	union {
		float f;
		Bit32u l;
	}	blah;
...
static INLINE void FPU_FST_F64(PhysPt addr) {
	mem_writed(addr,fpu.regs[TOP].l.lower);
	mem_writed(addr+4,fpu.regs[TOP].l.upper);
}

static INLINE void FPU_FST_F80(PhysPt addr) {
	FPU_ST80(addr,TOP);
}

static INLINE void FPU_FST_I16(PhysPt addr) {
	mem_writew(addr,static_cast<Bit16s>(FROUND(fpu.regs[TOP].d)));
}

static INLINE void FPU_FST_I32(PhysPt addr) {
	mem_writed(addr,static_cast<Bit32s>(FROUND(fpu.regs[TOP].d)));
}

static INLINE void FPU_FST_I64(PhysPt addr) {
	FPU_Reg blah;
...
static INLINE void FPU_FBST(PhysPt addr) {
...
static INLINE void FPU_FADD(Bitu op1, Bitu op2){
	fpu.regs[op1].d+=fpu.regs[op2].d;
	//flags and such :)
	return;
}

static INLINE void FPU_FSIN(void){
	//single precision
	if((fpu.cw & 0x300) == 0)
	{
...
static INLINE void FPU_FSINCOS(void){
...
static INLINE void FPU_FCOS(void){
	//single precision
			if((fpu.cw & 0x300) == 0)
			{
...
static INLINE void FPU_FSQRT(void){
	//single precision
				if((fpu.cw & 0x300) == 0)
				{
...
static INLINE void FPU_FPATAN(void){
	//single precision
	if((fpu.cw & 0x300) == 0)
	{
...
static INLINE void FPU_FPTAN(void){
	//single precision
	if((fpu.cw & 0x300) == 0)
	{
	...
	{
		fpu.regs[TOP].d = tanf_neon(fpu.regs[TOP].d);
	}
	FPU_PUSH(1.0);
	FPU_SET_C2(0);
	//flags and such :)
...
static INLINE void FPU_FDIV(Bitu st, Bitu other){
	fpu.regs[st].d= fpu.regs[st].d/fpu.regs[other].d;
	//flags and such :)
	return;
}

static INLINE void FPU_FDIVR(Bitu st, Bitu other){
	fpu.regs[st].d= fpu.regs[other].d/fpu.regs[st].d;
	// flags and such :)
	return;
}

static INLINE void FPU_FMUL(Bitu st, Bitu other){
	fpu.regs[st].d*=fpu.regs[other].d;
	//flags and such :)
	return;
}

static INLINE void FPU_FSUB(Bitu st, Bitu other){
	fpu.regs[st].d = fpu.regs[st].d - fpu.regs[other].d;
	//flags and such :)
	return;
}

static INLINE void FPU_FSUBR(Bitu st, Bitu other){
	fpu.regs[st].d= fpu.regs[other].d - fpu.regs[st].d;
	//flags and such :)
	return;
}

static INLINE void FPU_FXCH(Bitu st, Bitu other){
...
static INLINE void FPU_FST(Bitu st, Bitu other){
	fpu.tags[other] = fpu.tags[st];
	fpu.regs[other] = fpu.regs[st];
}


static INLINE void FPU_FCOM(Bitu st, Bitu other){
...
static INLINE void FPU_FPREM1(void){
	Real64 valtop = fpu.regs[TOP].d;
	Real64 valdiv = fpu.regs[STV(1)].d;
...
static INLINE void FPU_FXAM(void){
	if(fpu.regs[TOP].ll & LONGTYPE(0x8000000000000000))	//sign
	{ 
		FPU_SET_C1(1);
	} 
...
static INLINE void FPU_F2XM1(void){
	fpu.regs[TOP].d = pow((Real64)2.0,fpu.regs[TOP].d) - 1;
	return;
}

static INLINE void FPU_FYL2X(void){
	fpu.regs[STV(1)].d*=log(fpu.regs[TOP].d)/log(static_cast<Real64>(2.0));
	FPU_FPOP();
	return;
}

static INLINE void FPU_FYL2XP1(void){
	fpu.regs[STV(1)].d*=log(fpu.regs[TOP].d+1.0)/log(static_cast<Real64>(2.0));
	FPU_FPOP();
	return;
}

static INLINE void FPU_FSCALE(void){
	fpu.regs[TOP].d *= pow((Real64)2.0,static_cast<Real64>(static_cast<Bit64s>(fpu.regs[STV(1)].d)));
	return; //2^x where x is chopped.
}

static INLINE void FPU_FSTENV(PhysPt addr){
	FPU_SET_TOP(TOP);
	if(!cpu.code.big) {
...
static INLINE void FPU_FLDENV(PhysPt addr){
	Bit16u tag;
	Bit32u tagbig;
	Bitu cw;
	if(!cpu.code.big) {
...
static INLINE void FPU_FSAVE(PhysPt addr){
	FPU_FSTENV(addr);
...
static INLINE void FPU_FRSTOR(PhysPt addr){
	FPU_FLDENV(addr);
	Bitu start = (cpu.code.big?28:14);
	for(Bitu i = 0;i < 8;i++){
...
static INLINE void FPU_FXTRACT(void) {
...
//	Bit64s exp80final = (exp80>>52) - BIAS64;
	...
	Real64 mant = test.d / (pow((Real64)2.0,static_cast<Real64>(exp80final)));
	fpu.regs[TOP].d = static_cast<Real64>(exp80final);
	FPU_PUSH(mant);
}

static INLINE void FPU_FCHS(void){
	fpu.regs[TOP].d = -1.0*(fpu.regs[TOP].d);
}

static INLINE void FPU_FABS(void){
	fpu.regs[TOP].d = fabs(fpu.regs[TOP].d);
}

static INLINE void FPU_FTST(void){
	fpu.regs[8].d = 0.0;
	FPU_FCOM(TOP,8);
}

static INLINE void FPU_FLD1(void){
	FPU_PREP_PUSH();
	fpu.regs[TOP].d = 1.0;
}

static INLINE void FPU_FLDL2T(void){
	FPU_PREP_PUSH();
	fpu.regs[TOP].d = L2T;
}

static INLINE void FPU_FLDL2E(void){
	FPU_PREP_PUSH();
	fpu.regs[TOP].d = L2E;
}

static INLINE void FPU_FLDPI(void){
	FPU_PREP_PUSH();
	fpu.regs[TOP].d = PI;
}

static INLINE void FPU_FLDLG2(void){
	FPU_PREP_PUSH();
	fpu.regs[TOP].d = LG2;
}

static INLINE void FPU_FLDLN2(void){
	FPU_PREP_PUSH();
	fpu.regs[TOP].d = LN2;
}

static INLINE void FPU_FLDZ(void){
	FPU_PREP_PUSH();
	fpu.regs[TOP].d = 0.0;
