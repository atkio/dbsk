...
#define MIXER_AddSamples_Stereo(__data__) \
	int32x2_t diff_v; \
	int32x2_t work_v; \
	int32x2_t volmul_v=vld1_s32(volmul); \
	int32x2_t last_v=vld1_s32(last); \
	Bits data_int32[2]; \
	...
	for (;;) { \
		...
		if (pos<new_pos) { \
			last_v = vadd_s32(last_v,diff_v); \
			pos=new_pos; \
thestart: \
			if (pos>=len) return; \
			__data__; \
			diff_v = vsub_s32(vld1_s32(data_int32),last_v); \
		} \
		Bits diff_mul=freq_index & MIXER_REMAIN; \
		freq_index+=freq_add; \
		mixpos&=MIXER_BUFMASK; \
		work_v = vmul_s32(diff_v,vdup_n_s32(diff_mul)); \
		work_v = vadd_s32(last_v,vshr_n_s32(work_v ,MIXER_SHIFT)); \
		work_v = vmla_s32(vld1_s32(mixer.work[mixpos]),work_v,volmul_v); \
		vst1_s32(mixer.work[mixpos],work_v); \
		mixpos++;done++; \
	} \
	vst1_s32(last, last_v);
...
thestart:
		...
		if (stereo)
		{
			Bits samp[2]= {sample,last[1]+((diff[1]*diff_mul) >> MIXER_SHIFT)};
			sample_v =vld1_s32(samp);
		}
		else
		{
			sample_v =vdup_n_s32(sample);
		}
		work_v= vld1_s32(mixer.work[mixpos]);
		work_v= vadd_s32(work_v,vmul_s32(volmul_v,sample_v));
		vst1_s32(mixer.work[mixpos], work_v);
#else
...
#if HAVE_NEON
		work_v= vld1_s32(mixer.work[mixpos]);
		work_v= vadd_s32(work_v,vmul_s32(volmul_v,vdup_n_s32(sample)));
		vst1_s32(mixer.work[mixpos], work_v);
...
#if HAVE_NEON
	MIXER_AddSamples_Stereo(data_int32[0]=(((Bit8s)(data[pos*2+0] ^ 0x80)) << 8);data_int32[1]=(((Bit8s)(data[pos*2+1] ^ 0x80)) << 8););
#else
...
#if HAVE_NEON
	MIXER_AddSamples_Stereo(data_int32[0]=(data[pos*2+0]  << 8);data_int32[1]=(data[pos*2+1] << 8););
#else
...
#if HAVE_NEON
	MIXER_AddSamples_Stereo(data_int32[0]=data[pos*2+0];data_int32[1]=data[pos*2+1];);
#else
...
#if HAVE_NEON
	MIXER_AddSamples_Stereo(data_int32[0]=(Bits)data[pos*2+0]-32768;data_int32[1]=(Bits)data[pos*2+1]-32768;);
#else
...
#if HAVE_NEON
	MIXER_AddSamples_Stereo(data_int32[0]=(Bit16s)host_readw((HostPt)&data[pos*2+0]);data_int32[1]=(Bit16s)host_readw((HostPt)&data[pos*2+1]););
#else
...
#if HAVE_NEON
	MIXER_AddSamples_Stereo(data_int32[0]=(Bits)host_readw((HostPt)&data[pos*2+0])-32768;data_int32[1]=(Bits)host_readw((HostPt)&data[pos*2+1])-32768;);
#else
...
#if HAVE_NEON
	MIXER_AddSamples_Stereo(data_int32[0]=(Bit32s)host_readd((HostPt)&data[pos*2+0]);data_int32[1]=(Bit32s)host_readd((HostPt)&data[pos*2+1]););
#else
...
static void MIXER_MixData(Bitu needed) {
	...
	if (CaptureState & (CAPTURE_WAVE|CAPTURE_VIDEO)) {
		...
		for (Bitu i=0;i<added;i++) {
			int32x2_t sample= vld1_s32(mixer.work[readpos]);
			sample = vshr_n_s32(sample,MIXER_VOLSHIFT);
			sample = vmax_s32(vdup_n_s32(MIN_AUDIO),sample);
			sample = vmin_s32(vdup_n_s32(MAX_AUDIO),sample);
			convert[i][0] = vget_lane_s32(sample,0);
			convert[i][1] = vget_lane_s32(sample,1);
...
//		LOG_MSG("overflow run need %d, have %d, min %d", need, mixer.done, mixer.min_needed);
	...
	if(need != reduce) {
		while (need--) {
			...
			if (enableMixerHack) {
#if HAVE_NEON
				int32x2_t sample_v= vld1_s32(mixer.work[i]);
				sample_v = vshr_n_s32(sample_v,MIXER_VOLSHIFT);
				sample_v = vmax_s32(vdup_n_s32(MIN_AUDIO),sample_v);
				sample_v = vmin_s32(vdup_n_s32(MAX_AUDIO),sample_v);
				*output++ = (vget_lane_s32(sample_v,0) + vget_lane_s32(sample_v,1))/2;
#else
				*output++ = (MIXER_CLIP(mixer.work[i][0]>>MIXER_VOLSHIFT)+MIXER_CLIP(mixer.work[i][1]>>MIXER_VOLSHIFT))/2;
				...
				sample_v = vmax_s32(vdup_n_s32(MIN_AUDIO),sample_v);
				sample_v = vmin_s32(vdup_n_s32(MAX_AUDIO),sample_v);
				*output++ = vget_lane_s32(sample_v,0);
				*output++ = vget_lane_s32(sample_v,1);
#else
		...
		while (reduce--) {
			...
			if (enableMixerHack) {
#if HAVE_NEON
				int32x2_t sample_v= vld1_s32(mixer.work[pos]);
				sample_v = vshr_n_s32(sample_v,MIXER_VOLSHIFT);
				sample_v = vmax_s32(vdup_n_s32(MIN_AUDIO),sample_v);
				sample_v = vmin_s32(vdup_n_s32(MAX_AUDIO),sample_v);
				*output++ = (vget_lane_s32(sample_v,0) + vget_lane_s32(sample_v,1))/2;
...
void MIXER_Init(Section* sec) {
	...
	if (enableMixerHack) {
		// limit freq to 11025hz with mixerHack
