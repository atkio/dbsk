...
/*
extern "C" void Java_com_fishstix_dosbox_DosBoxControl_nativeMouseWarp(JNIEnv * env, jobject obj, jint x, jint y, jfloat xfactor, jfloat yfactor, jint src_left, jint src_right, jint src_top, jint src_bottom, jint dst_left, jint dst_right, jint dst_top, jint dst_bottom) {
	struct locnet_al_event	event;
		...
		if ((mixerCallBack != 0) && (loadf->audioBuffer != 0)) {
			...
			/*for (int i=0;i<len/2;i++) {
				if (flip)
					...
					audioBuffer[i] = audioBuffer[i*2+1];
			}*/
			//}
